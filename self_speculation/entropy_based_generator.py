...
from self_speculation.llama_model_utils import (
    ...
    forward_early,
    forward_remainder,
    determine_entropy_shared_exit_layer
)
import random
...
class EntropyExitGenerationStrategy(GenerationStrategy):
    def generate_token_ids(
        ...
        while len(output_ids) < generation_config.max_steps:
            ...
            eos_found = False
            exit_layers.append(current_exit_layer)
    ...
    def single_step_speculation(
        ...
        #print('original num_speculations', num_speculations)
        # Determine exit layer based on entropy threshold
        #print(colorama.Fore.LIGHTMAGENTA_EX, end="")
        exit_layer = determine_entropy_shared_exit_layer(
            model=model,
            input_ids=input_ids,
            entropy_threshold=entropy_threshold,
            num_speculations=num_speculations,
        )
        #print("for _ in range(num_speculations)", num_speculations)
        #print(exit_layer)
        ...
        if streamer:
            if isinstance(streamer, SpeculativeTextStreamer):
                #(colorama.Fore.LIGHTMAGENTA_EX, end="")
